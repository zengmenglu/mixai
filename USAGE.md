...
```
┌──────────────────────────────────────────────────────────────┐
│ mixai  DeepSeek·Doubao·Kimi·ChatGPT   [☑DeepSeek ☑Doubao ☑Kimi ☑ChatGPT]  ＋新会话  ● │
├────────────┬────────────┬────────────┬────────────┐          │
│  DeepSeek  │   Doubao   │    Kimi    │   ChatGPT  │          │
│  [回答区]   │  [回答区]   │   [回答区]  │   [回答区]  │          │
│            │            │            │            │          │
│  [✕终止回答] │  [✕终止回答] │  [✕终止回答] │  [✕终止回答] │ ← 仅回答中显示 │
└────────────┴────────────┴────────────┴────────────┘          │
│ [            输入一个问题，回车同时问四个模型            ] [发送] │
└──────────────────────────────────────────────────────────────┘
```
...
| **终止某个慢模型** | 该栏底部的「✕ 终止回答」按钮（仅回答中显示）。终止后该栏不再占用，你可以继续问下一个问题，其他模型不受影响 |
| **开新会话** | 右上「＋ 新会话」清空所有栏的历史，下轮各开新对话 |
| **重新登录某栏** | 该栏显示 `logged-out` 时，点栏内「打开登录窗口」 |

### 终止按钮的语义（重要）

- 点「✕ 终止回答」会**立即停止抓取该模型的回答**，并点击该网站自己的「停止生成」按钮
  （释放输入框，方便下一轮提问）。
- 被终止的栏状态变为 `stopped`，已显示的部分回答会保留。
- **其他模型继续回答，互不阻塞**；被终止的栏下一轮也能正常提问。
...
backend/
  ...
  orchestrator.js          ← 并发分发 + 故障隔离 + stop/abort
  transport.js             ← SSE 事件总线
  log.js                   ← 结构化日志模块
  login.js                 ← 登录/恢复流程
  browser/contextFactory.js ← 浏览器启动 + stealth
  browser/scrape.js        ← 流式抓取 + 完成判定 + 重复拦截
  adapters/<provider>.js   ← 每家一个：选择器表 S + 交互
web/
  index.html app.js styles.css ← 前端四栏 UI
