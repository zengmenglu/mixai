...
每轮回答下方两个小图标，只作用于该模型，互不相干：

- **⏹ 停止**（回答中显示）-> `POST /api/stop/:id` -> `Orchestrator.stop(id)` 调 `controller.abort()` 中断该 pane 的 `streamAnswer`，并调 `adapter.stopGenerating()` 点击该网站停止按钮释放输入框。状态变 `stopped`，不阻塞其他 pane。
- **↻ 重新回答**（回答完成后显示）-> 前端 `regenerate(id)` 清空该栏当前回答，以 `{question: 上一问, providers:[id], resumeUrls}` 复用 `POST /api/ask` 重新发送同一问题，新回答**替换**原回答。`streamAnswer` 的 baseline 机制确保不重发旧答案。各模型自带的「重新生成」按钮因纯图标/无文字/混淆 class 难以稳定定位，故弃用，改用重发+替换（四家通用）。

### 回答渲染（原生 HTML）

各 adapter 的 `readLatestAnswerHtml()` 返回最新回答的 innerHTML（含 `<p>/<ul>/<strong>/<a>/<table>` 等原生结构），`streamAnswer` 用 text-stability 判完成、以 HTML 为流式载荷（每个 delta 是当前完整 HTML，前端整体替换）。前端 `renderAnswer` 直接 `sanitizeHtml` 后渲染（剔 `script/iframe/on*/style/javascript:`），保留各平台原生格式（分段、列表、加粗、链接、编号、表格）。DeepSeek 额外剥 `.ds-markdown-cite` 引用标注；豆包取最后一个有内容的 `[data-render-engine]` 块（跳过搜索状态条）；Kimi 取最后一个有内容的 `.markdown`（跳过思考段）。
